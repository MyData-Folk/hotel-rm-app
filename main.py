...
async def process_excel_file(session: Session, hotel_id: str, file_content: bytes, filename: str):
    ...
    try:
        ...
        serialized_data = json.dumps(parsed, indent=2, ensure_ascii=False)
        existing_data = session.exec(select(HotelData).where(HotelData.hotel_id == hotel_id)).first()

        if existing_data:
            existing_data.data_json = serialized_data
            existing_data.updated_at = datetime.utcnow()
            logger.info(f"Données mises à jour en BDD pour {hotel_id}")
        else:
            db_data = HotelData(hotel_id=hotel_id, data_json=serialized_data)
            session.add(db_data)
            logger.info(f"Données insérées en BDD pour {hotel_id}")
...
class HotelData(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    hotel_id: str = Field(index=True, unique=True)
    data_json: str
    updated_at: datetime = Field(default_factory=datetime.utcnow)
...
def backup_immediate(performed_by: str = Query("system", alias="actor")):
    ...
    with Session(engine) as session:
        ...
        with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as archive:
            for hotel in hotels:
                data_record = session.exec(select(HotelData).where(HotelData.hotel_id == hotel.hotel_id)).first()
                if data_record:
                    archive.writestr(f"data/{hotel.hotel_id}.json", data_record.data_json)
                    data_files_written += 1
                    total_bytes += len(data_record.data_json.encode('utf-8'))
...
def get_data(hotel_id: str = Query(...)):
    ...
    with Session(engine) as session:
        db_data = session.exec(select(HotelData).where(HotelData.hotel_id == hotel_id)).first()
        ...
        try:
            data = json.loads(db_data.data_json)
            logger.info(f"Données chargées depuis la BDD pour {hotel_id}")
            return data
        except Exception as e:
            logger.error(f"Erreur lecture données BDD pour {hotel_id}: {str(e)}")
    ...
    try:
        ...
        with Session(engine) as session:
            log_activity(
                ...
                description=f"Echec consultation disponibilités pour {hotel_id}",
                hotel_id=request.hotel_id,
                performed_by=(request.performed_by or "system"),
                details={"error": str(e)},
            )
...
def check_files_status(hotel_id: str = Query(...)):
    ...
    with Session(engine) as session:
        data_exists = session.exec(select(HotelData).where(HotelData.hotel_id == hotel_id)).first() is not None
